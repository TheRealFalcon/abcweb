...
function Abc(user) {
    ...
    function y_get(st, up, x, w) {
        var y, p_staff = staff_tb[st], i = x / realwidth * YSTEP | 0, j = (x + w) / realwidth * YSTEP | 0;
        if (i < 0)
            i = 0;
        if (j >= YSTEP) {
            j = YSTEP - 1;
    ...
    function d_arp(de) {
        var m, h, dx, s = de.s, dd = de.dd, xc = 5;
        if (s.type == NOTE) {
            for (m = 0; m <= s.nhd; m++) {
                if (s.notes[m].acc) {
                    dx = 5 + s.notes[m].shac
    ...
    function d_cresc(de) {
        if (de.ldst)
            return;
        var s, dd, dd2, up, x, dx, x2, i, s2 = de.s, de2 = de.start, de2_prev, de_next;
        s = de2.s;
        x = s.x + 3;
        i = de2.ix;
        if (i > 0)
    ...
    function d_trill(de) {
        if (de.ldst)
            return;
        var dd, up, y, w, tmp, s2 = de.s, st = s2.st, s = de.start.s, x = s.x;
    ...
    function deco_cnv(a_dcn, s, prev) {
        ...
        for (i in a_dcn) {
            ...
            case 2:
                if (!s.notes) {
                    error(1, s, "!$1! must be on a note or a rest", dd.name);
                    continue
                }
                break;
            case 8:
            ...
            case 9:
                if (!s.notes) {
                    error(1, s, "!$1! must be on a note or rest", dd.name);
                    continue
                }
                for (j = 0; j <= s.nhd; j++) {
                    note = s.notes[j];
    ...
    function draw_deco_staff() {
        var s, first_gchord, p_voice, x, y, w, i, v, de, dd, gch, gch2, ix, top, bot, minmax = new Array(nstaff);
        function draw_repbra(p_voice) {
            ...
            for (; s; s = s.next) {
                ...
                if (s1.text)
                    xy_str(x + 4, y2 - gene.curfont.size * .9, s1.text);
                xypath(x, y2);
                if (s1.rbstart == 2)
    ...
    function draw_measnb() {
        var s, st, bar_num, x, y, w, any_nb, font_size, sy = cur_sy;
    ...
    function draw_partempo(st, top) {
        var s, some_part, some_tempo, h, w, y, dy = 0, ht = 0;
    ...
    function calculate_beam(bm, s1) {
        var s, s2, notes, nflags, st, v, two_staves, two_dir, hh, x, y, ys, a, b, stem_err, max_stem_err, sx, sy, sxx, sxy, syy, a0, stem_xoff, scale, visible, dy;
        if (!s1.beam_st) {
            s = sym_dup(s1);
            s.prev = s1.prev;
            if (s.prev)
        ...
        if (two_dir) {
            ...
            for (; s && s.time <= s2.time; s = s.ts_next) {
                if (s.type != NOTE || s.invis || s.st != st && s.v != v) {
                    continue
                }
                x = s.v == v ? s.xs : s.x;
    ...
    function draw_hl(x, yl, yu, st, hltype) {
        var i, p_staff = staff_tb[st], staffb = p_staff.y, top = (p_staff.stafflines.length - 1) * 6;
        yl = Math.ceil(yl / 6) * 6;
    ...
    function draw_bar(s, bot, h) {
        var i, s2, yb, bar_type, st = s.st, x = s.x;
        if (s.bar_mrep) {
            yb = staff_tb[st].y + 12;
            set_scale(s);
            if (s.bar_mrep == 1) {
                ...
                if (s.v == cur_sy.top_voice) {
                    set_font("annotation");
                    xy_str(x, yb + staff_tb[st].topbar - 9, s.bar_mrep.toString(), "c")
                }
            }
        }
        if (st != 0 && s.ts_prev && s.ts_prev.type != BAR)
    ...
    function draw_rest(s) {
        var s2, i, j, x, y, dotx, staffb, yb, yt, head, p_staff = staff_tb[s.st];
        if (p_staff.empty)
            return;
        ...
        if (s.rep_nb) {
            ...
            if (s.rep_nb < 0) {
                ...
                if (s.rep_nb > 2 && s.v == cur_sy.top_voice) {
                    set_font("annotation");
                    xy_str(x, staffb + p_staff.topbar - 9, s.rep_nb.toString(), "c")
                }
            }
            return
        }
    ...
    function draw_gracenotes(s) {
        ...
        if (s.p_v.key.k_bagpipe || !cfmt.graceslurs || s.slur_start || !s.next || s.next.type != NOTE)
            return;
        last = g;
        if (last.stem >= 0) {
            yy = 127;
    ...
    function draw_basic_note(x, s, m, y_tb) {
        var i, k, y, p, yy, dotx, doty, old_color = false, note = s.notes[m], staffb = staff_tb[s.st].y, y = 3 * (note.pit - 18), shhd = note.shhd * stv_g.scale, x_note = x + shhd, y_note = y + staffb;
        if (s.invis)
            return;
        ...
        if (note.invis) {} else if (note.head) {
            p = note.head
        } else if (s.grace) {
            p = "ghd";
    ...
    function draw_tuplet(s1, lvl) {
        var s2, s3, g, upstaff, nb_only, some_slur, x1, x2, y1, y2, xm, ym, a, s0, yy, yx, dy, a, b, dir, p, q, r;
        function tuplet_done() {
            if (lvl == 0)
                s1.tp0 = 0;
            else
                s1.tp1 = 0
        ...
        if (s1 == s2) {
            ...
            if (s1.tf[0] == 2 || s1.type != NOTE || s2.type != NOTE) {
                nb_only = false
            } else {
                nb_only = true;
    ...
    function draw_all_slurs(p_voice) {
        var k, i, m2, s = p_voice.sym, slur_type = p_voice.slur_start, slur_st = 0;
        if (!s)
            return;
        if (slur_type) {
    ...
    function draw_vname(indent) {
        var p_voice, n, st, v, a_p, p, y, name_type, staff_d = [];
    ...
    function draw_systems(indent) {
        var s, s2, st, x, x2, res, staves_bar, bar_force, xstaff = [], bar_bot = [], bar_height = [];
        function bar_set() {
        ...
        function draw_staff(st, x1, x2) {
            var w, i, dx, y = staff_tb[st].y, stafflines = staff_tb[st].stafflines, l = stafflines.length;
            for (i = 0; i < l; i++) {
                if (stafflines[i] != ".") {
                    set_sscale(st);
                    w = (x2 - x1) / stv_g.scale;
                    ...
                    for (i++; i < l; i++) {
                        y -= 6;
                        if (stafflines[i] != ".") {
                            output.push("m-" + w.toFixed(2) + " " + y + "h" + w.toFixed(2));
                            y = 0
                        }
                    }
                    output.push('"/>\n');
    ...
    function draw_symbols(p_voice) {
        ...
        first_note = true;
        for (s = p_voice.sym; s; s = s.next) {
            if (s.invis && s.type != NOTE && s.type != REST && s.type != GRACE)
                continue;
            x = s.x;
            set_color(s.color);
            switch (s.type) {
    ...
    var musicfont = 'url("data:application/font-woff;base64,")';
    var defined_font = {}
      , font_tb = {}
      , fid = 1
      , font_scale_tb = {
    ...
    function get_bool(param) {
        return !param || !param.match(/^(0|n|f)/i)
    }
    ...
    function tosvg(in_fname, file) {
        var i, c, bol, eol, end, ext, select, skip, line0, line1, last_info, opt, text, a, b, s, cfmt_sav, info_sav, char_tb_sav, glovar_sav, maps_sav, mac_sav, pscom, txt_add = "\n", eof = file.length;
        ...
        for (bol = 0; bol < eof; bol = parse.eol + 1) {
            ...
            if (pscom) {
                ...
                if (b) {
                    ...
                    if (i < 0) {
                        syntax(1, "No $1 after %%$2", end.slice(1), b[0]);
                        parse.eol = eof;
                        continue
                    }
                    do_begin_end(b[1], a[1], file.slice(eol + 1, i).replace(new RegExp("^" + line0 + line1,"gm"), ""));
            ...
            text = uncomment(file.slice(bol + 2, eol).trim(), true);
            if (line0 == "+") {
                if (!last_info) {
    ...
    var gene, staff_tb, nstaff, tsnext, realwidth, insert_meter, beta_last, space_tb = [7, 10, 14.15, 20, 28.3, 40, 56.6, 80, 113, 150], smallest_duration;
    const dx_tb = [10, 10, 11, 13, 13];
    const hw_tb = [4.5, 5, 6, 7, 8];
    ...
    function set_graceoffs(s) {
        var next, m, gspleft = Number(cfmt.gracespace[0]), gspinside = Number(cfmt.gracespace[1]), gspright = Number(cfmt.gracespace[2]), xx = 0, g = s.extra;
        g.beam_st = true;
        for (; ; g = g.next) {
            set_head_shift(g);
            for (m = g.nhd; m >= 0; m--) {
    ...
    function gchord_width(s, wlnote, wlw) {
        var s2, gch, w, wl, ix, lspc = 0, rspc = 0, alspc = 0, arspc = 0;
        ...
        while (1) {
            ...
            do {
                ...
                if (new_val > maxx)
                    maxx = new_val;
                s = s.ts_next
            } while (s != last_s && !s.seqst);s2.shrink = maxx - xa;
            if (s2.ts_prev)
    ...
    function get_ck_width() {
        ...
        if (!cfmt.singleline)
            return p_voice.clef.wl + p_voice.clef.wr + p_voice.key.wl + p_voice.key.wr;
    ...
    function get_width(first, last, w) {
        ...
        for (s = first; s != last; s = s.ts_next) {
            ...
            if ((space = s.space) < shrink)
                ...
                w += shrink * cfmt.maxshrink + space * sp_fac
        }
        return w
    }
    function set_lines(s, last, lwidth, indent) {
        var first, s2, s3, x, xmin, xmax, wwidth, shrink, space, nlines, cut_here;
        ...
        while (1) {
            ...
            if (s.type == BAR)
                cut_here = true;
            if (!cut_here) {
                var beam = s2.dur && !s2.beam_st && !s2.beam_end
                  , bar_time = s2.time;
                s = s2;
                s2 = s3 = null;
                xmax -= 6;
    ...
    function set_clefs() {
        var s, s2, st, v, p_voice, g, new_type, new_line, p_staff, pit, staff_clef = new Array(nstaff), sy = cur_sy;
    ...
    const rest_sp = [[18, 18], [12, 18], [12, 12], [0, 12], [6, 8], [10, 10], [6, 4], [10, 0], [10, 4], [10, 10]];
    function set_pitch(last_s) {
        var s, s2, g, st, delta, m, pitch, note, dur = BASE_LEN, staff_delta = new Array(nstaff), sy = cur_sy;
        for (st = 0; st <= nstaff; st++) {
            s = staff_tb[st].clef;
    ...
    function set_stem_dir() {
        ...
        while (s) {
            ...
            for (u = s; u; u = u.ts_next) {
                ...
                if (u.type != NOTE && u.type != REST || u.invis)
                    continue;
                st = u.st;
                if (st > nst) {
    ...
    function init_music_line() {
        var p_voice, s, s2, last_s, v, st, nv = voice_tb.length;
        for (v = 0; v < nv; v++) {
        ...
        for (v = 0; v < nv; v++) {
            if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || !cur_sy.st_print[cur_sy.voices[v].st])
                continue;
            p_voice = voice_tb[v];
    ...
    function set_words(p_voice) {
        var s, s2, nflags, lastnote, start_flag = true, pitch = 127;
        for (s = p_voice.sym; s; s = s.next) {
            if (s.type == NOTE) {
                pitch = s.notes[0].pit;
                break
        ...
        for (s = p_voice.sym; s; s = s.next) {
            ...
            case BAR:
                if (!s.beam_on)
                    start_flag = true;
                if (!s.next && s.prev && s.prev.head == OVALBARS)
                    s.prev.head = SQUARE;
                break;
            case NOTE:
            case REST:
    ...
    function set_beams(sym) {
        var s, t, g, beam, s_opp, dy, avg, n, laststem = -1, lasty = 0;
        for (s = sym; s; s = s.next) {
            if (s.type != NOTE) {
                if (s.type != GRACE)
                    continue;
    ...
    function same_head(s1, s2) {
        ...
        do {
            ...
            if (sh2 < s2.notes[i2].shhd)
                sh2 = s2.notes[i2].shhd
        } while (s2.notes[i2].pit == s1.notes[i1].pit);if (i1 <= s1.nhd) {
            if (i2 <= s2.nhd)
                return false;
            if (s2.stem > 0)
                return false
    ...
    function set_overlap() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            while (1) {
                ...
                if (s2.type == NOTE && !s2.invis && s2.st == s.st)
                    break
            }
            if (!s2)
                continue;
            ...
            left2 = set_left(s2);
            s3 = s1.ts_prev;
            if (s3 && s3.time == s1.time && s3.st == s1.st && s3.type == NOTE && !s3.invis) {
                right3 = set_right(s3);
                for (i = 0; i < MAXPIT; i++) {
    ...
    function check_bar(s) {
        var bar_type, i, b1, b2, p_voice = s.p_v;
    ...
    function set_piece() {
        var s, p_voice, st, v, nst, nv, non_empty = [], non_empty_gl = [], sy = cur_sy;
    ...
    function set_sym_glue(width) {
        ...
        var some_grace, s = tsfirst, xmin = 0, x = 0;
        while (1) {
            if (s.type == GRACE)
                some_grace = true;
            if (s.seqst) {
        ...
        if (tsnext) {
            if (x >= width) {
                ...
                if (beta_last > beta0) {
                    if (cfmt.stretchstaff) {
                        if (cfmt.linewarn) {
                            error(0, s, "Line underfull ($1pt of $2pt)", (beta0 * xmax + (1 - beta0) * x).toFixed(2), width.toFixed(2))
                        }
                    } else {
                        width = x;
                        beta_last = 0
        ...
        if (x >= width) {
            ...
            if (x == xmin) {
                ...
                alfa = (x - width) / (x - xmin);
                if (alfa > 1) {
                    error(0, s, "Line too much shrunk $1 $2 $3", xmin.toFixed(2), x.toFixed(2), width.toFixed(2))
                }
            }
        ...
        if (some_grace) {
            for (s = tsfirst; s; s = s.ts_next) {
                if (s.type != GRACE)
                    continue;
                if (s.gr_shift)
                    x = s.prev.x + s.prev.wr + Number(cfmt.gracespace[0]);
                else
    ...
    function set_user(parm) {
        var k, a = parm.match(/(.*?)[= ]+(.*)/), c = a[1], v = a[2];
        if (!v || v[0] != "!" && v[0] != '"') {
    ...
    function new_key(param) {
        var i, clef, key_end, c, tmp, mode = 0, s = {
            type: KEY,
            ctx: parse.ctx,
            istart: parse.istart,
            iend: parse.iend,
    ...
    function new_meter(text) {
        ...
        if (parse.state != 3) {
            ...
            if (parse.state >= 1) {
                if (!glovar.ulen) {
                    if (wmeasure <= 1 || wmeasure >= BASE_LEN * 3 / 4)
    ...
    function new_tempo(text) {
        ...
        if (c == "=") {
            ...
            while (c == " ")
                c = text[++tmp.index];
            i = tmp.index;
            if (c == "c" && text[i + 1] == "a" && text[i + 2] == "." && text[i + 3] == " ") {
                s.tempo_ca = "ca. ";
                tmp.index += 4;
                c = text[tmp.index]
            }
    ...
    function new_bar() {
        var s2, c, bar_type, line = parse.line, s = {
            type: BAR,
            ctx: parse.ctx,
        ...
        if (s2 && s2.type == KEY && (!s2.prev || s2.prev.type != BAR)) {
            curvoice.last_sym = s2.prev;
            if (!s2.prev)
                curvoice.sym = s2.prev;
            sym_link(s);
            s.next = s2;
            s2.prev = s;
    ...
    function parse_staves(p) {
        var v, vid, a_vf = [], err = false, flags = 0, brace = 0, bracket = 0, parenth = 0, flags_st = 0, i = 0;
        while (i < p.length) {
            switch (p[i]) {
            case " ":
            case "\t":
    ...
    function set_map(note) {
        var bn, an, nn, i, nmap, map = maps[curvoice.map];
    ...
    function note_transp(s) {
        var i, j, n, d, a, acc, i1, i3, i4, note, m = s.nhd, sf_old = curvoice.okey.k_sf, i2 = curvoice.ckey.k_sf - sf_old, dp = cgd2cde[(i2 + 4 * 7) % 7], t = curvoice.vtransp;
        if (t < 0 && dp != 0)
            dp -= 7;
        dp += (t / 3 / 12 | 0) * 7;
        for (i = 0; i <= m; i++) {
            ...
            a = acc2[i1];
            if (note.acc) {} else if (curvoice.ckey.k_none) {
                if (a == 3 || acc_same_pitch(note.pit))
                    continue
    ...
    function new_note(grace, tp_fact) {
        var note, s, in_chord, c, dcn, type, i, n, s2, nd, res, num, dur, sl1 = 0, line = parse.line, a_dcn_sav = a_dcn;
        a_dcn = null;
        ...
        case "z":
            ...
            line.index++;
            nd = parse_dur(line);
            s.dur_orig = (curvoice.ulen < 0 ? BASE_LEN / 4 : curvoice.ulen) * nd[0] / nd[1];
    ...
    function parse_music_line() {
        var s, grace, sappo, dcn, i, c, idx, type, n, text, last_note_sav, a_dcn_sav, no_eol, tp_a = [], tp, tpn = -1, tp_fact = 1, slur_start = 0, line = parse.line;
        ...
        while (1) {
            ...
            case "}":
                ...
                grace = null;
                if (!s.prev && !curvoice.key.k_bagpipe) {
                    for (i = 0; i <= s.nhd; i++)
                        s.notes[i].dur *= 2;
                    s.dur *= 2;
                    s.dur_orig *= 2;
    ...
    function out_str(str) {
        var n_font, o_font = gene.curfont, c_font = o_font;
    ...
    function xy_str(x, y, str, action, line_w) {
    ...
    function xy_str_b(x, y, str) {
        ...
        output.push('" width="' + (w + 4).toFixed(2) + '" height="' + (gene.curfont.size + 3).toFixed(2) + '"/>\n');
        xy_str(x, y, str)
    }
    ...
    function set_dscale(st, no_scale) {
        if (st < 0) {
            ...
            output = stv_g.scale == 1 ? staff_tb[st].output : staff_tb[st].sc_out
        }
        stv_g.st = st;
        stv_g.dy = 0
    }
    function delayed_update() {
        ...
        for (st = 0; st <= nstaff; st++) {
            if (staff_tb[st].sc_out.length != 0) {
                output.push('<g transform="translate(0,' + (posy - staff_tb[st].y).toFixed(2) + ") scale(" + staff_tb[st].staffscale.toFixed(2) + ')">\n');
    ...
    const anno_type = ["bar", "clef", "custos", "", "grace", "key", "meter", "Zrest", "note", "part", "rest", "yspace", "staves", "Break", "tempo", "", "block", "remark"];
    ...
    function g_open(x, y, rot, sx, sy) {
        ...
        if (sx) {
            if (sy)
                output.push(") scale(" + sx.toFixed(2) + ", " + sy.toFixed(2));
            else
    ...
    function out_bar(x, y, h, dotted) {
        output.push('<path class="stroke" stroke-width="1" ' + (dotted ? 'stroke-dasharray="5,5" ' : "") + 'd="m' + (x + posx).toFixed(2) + " " + (posy - y).toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n')
    }
    ...
    function out_thbar(x, y, h) {
        x += posx + 1.5;
        y = posy - y;
        output.push('<path class="stroke" stroke-width="3" d="m' + x.toFixed(2) + " " + y.toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n')
    }
    ...
    function out_8vb(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font:italic bold 12px serif">8<tspan dy="-4" style="font-size:10px">vb</tspan></text>\n', x - 8, y);
            x += 4;
            val -= 4
        } else {
            val -= 5
    ...
    function out_15ma(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font:italic bold 12px serif">15<tspan dy="-4" style="font-size:10px">ma</tspan></text>\n', x - 10, y);
            x += 20;
            val -= 20
        } else {
            val -= 5
    ...
    function out_15mb(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font:italic bold 12px serif">15<tspan dy="-4" style="font-size:10px">mb</tspan></text>\n', x - 10, y);
            x += 7;
            val -= 7
        } else {
            val -= 5
    ...
    function svg_flush() {
        ...
        if (cfmt.scale == 1)
            ...
            head += '<g class="music" stroke-width=".7" transform="scale(' + cfmt.scale.toFixed(2) + ')">\n';
        if (typeof svgobj == "object") {
            svgobj.setg(0);
            output.push(svgbuf);
            svgbuf = ""
        }
    ...
    const w_tb = [2, 1, 7, 0, 3, 5, 6, 9, 9, 0, 9, 3, 0, 7, 0, 0, 0, 0];
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, new_sy, nb, nv = voice_tb.length, vtb = [], vn = [], mrest_time = -1;
        for (v = 0; v < nv; v++)
    ...
    function do_clip() {
        ...
        do {
            ...
            if (!s)
                return
        } while (!s.seqst);for (v in voice_tb) {
            p_voice = voice_tb[v];
        ...
        for (; s; s = s.ts_next) {
            ...
            case BAR:
                ...
                do {
                    ...
                    s2 = s2.next
                } while (s2 && s2.time == tim);s.bar_num = bar_num;
                bar_time = s.time + wmeasure;
                break
            }
        }
    ...
    function set_transp() {
        ...
        if (curvoice.ckey.k_bagpipe || curvoice.ckey.k_drum)
            return;
        transp = (cfmt.transp || 0) + (curvoice.transp || 0) + (curvoice.shift || 0);
    ...
    function do_pscom(text) {
        var h1, val, s, cmd, param, n, k, b, lock = false;
        if (text.match(/ lock$/)) {
            lock = true;
        ...
        case "repeat":
            ...
            return;
        case "sep":
            var h2, len, values, lwidth = cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin;
            h1 = h2 = len = 0;
            if (param) {
    ...
    function get_vover(type) {
        ...
        function clone_voice(id) {
            ...
            p_voice.lyric_restart = p_voice.lyric_restart = p_voice.lyric_cont = p_voice.ly_a_h = null;
    ...
    function get_clef(s) {
        ...
        s2 = curvoice.last_sym;
        if (s2 && s2.prev && s2.time == curvoice.time && (s2.type == KEY && !s2.k_none || s2.type == BAR)) {
    ...
    function get_key(kp) {
        var v, p_voice, s, transp, s_key = kp[0], a = kp[1];
    ...
    function get_voice(parm) {
        var v, transp, vtransp, a = info_split(parm, 1), vid = a.shift();
        if (parse.state < 2) {
            if (a.length != 0)
                memo_kv_parm(vid, a);
            if (parse.state == 1)
        ...
        if (!curvoice.filtered && !curvoice.ignore && parse.voice_opts) {
    ...
    function goto_tune(is_K) {
        ...
        for (v in voice_tb) {
            ...
            if (p_voice.key.k_bagpipe && !p_voice.pos.stm) {
        ...
        if (staves_found < 0) {
            ...
            for (v = 0; v <= nstaff; v++) {
                ...
                p_voice.time = 0;
                p_voice.clef.time = 0;
                p_voice.st = p_voice.cst = par_sy.voices[v].st = par_sy.voices[v].range = v;
                par_sy.staves[v] = {
                    stafflines: "|||||",
                    staffscale: 1
                }
    ...
    function draw_lyric_line(p_voice, j, y) {
        var l, p, lastx, w, s, s2, f, ly, lyl, hyflag, lflag, x0, font, shift, desc;
    ...
    function draw_all_lyrics() {
        var p_voice, s, v, nly, i, x, y, w, a_ly, ly, lyst_tb = new Array(nstaff), h_tb = new Array(voice_tb.length), nly_tb = new Array(voice_tb.length), above_tb = new Array(voice_tb.length), rv_tb = new Array(voice_tb.length), top = 0, bot = 0, st = -1;
        for (v in voice_tb) {
            p_voice = voice_tb[v];
            if (!p_voice.sym)
                continue;
    ...
    function parse_gchord(type) {
        var c, text, gch, x_abs, y_abs, type, i, istart, iend, ann_font = get_font("annotation"), h_ann = ann_font.size, line = parse.line;
        function get_float() {
            var txt = "";
            while (1) {
                c = text[i++];
    ...
    function gch_transp(s) {
        var gch, p, i = 0, i2 = curvoice.ckey.k_sf - curvoice.okey.k_sf;
        function gch_tr1(p) {
    ...
    function gch_build(s) {
        var gch, w, xspc, ix, pos = curvoice.pos.gch == SL_BELOW ? -1 : 1, gch_font = get_font("gchord"), ann_font = get_font("annotation"), y_above = 0, y_below = 0, y_left = 0, y_right = 0, h_gch = gch_font.size, h_ann = ann_font.size, box = cfmt.gchordbox;
        s.a_gch = a_gch;
        a_gch = null;
        if (curvoice.vtransp)
            gch_transp(s);
        ...
        for (ix = 0; ix < n; ix++) {
            ...
            default:
                ...
                if (i >= 0) {
                    ...
                    if (user.anno_start)
                        user.anno_start("gchord", gch.istart, gch.iend, x - 2, y + h + 2, w + 4, h + 4, s);
                    i = 0;
                    j = i;
                    for (; ; ) {
                        i = text.indexOf("\t", j);
                    ...
                    if (user.anno_stop)
                        user.anno_stop("gchord", gch.istart, gch.iend, s.x - 2, y + h + 2, w + 4, h + 4, s);
                    continue
                }
                break;
            case "@":
